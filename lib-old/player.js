var Player = {
	...
	emit_online_players: function() {
		DB.query("SELECT `initials`, `team`, TIMESTAMPDIFF(SECOND, connected_at, last_ping) AS `score_seconds`" +
			" FROM `players`" +
	...
	update_initials: function(socket, initials) {
		...
		DB.query("UPDATE `players` SET `initials` = ? WHERE `id` = ?",
	...
	ping_player: function(socket) {
		...
		DB.query("UPDATE `players` SET `last_ping` = CURRENT_TIMESTAMP WHERE `id` = ?",
			[socket.player_id],
