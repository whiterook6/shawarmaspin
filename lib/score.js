Score = {
	...
	send_team_high_scores: function(){
		...
		for (var i = Game.players.length - 1; i >= 0; i--) {
			...
			add_to_team_scores(team_scores, team, score, spm);
		}
		...
		DB.query("SELECT `team`, SUM(`score`) as `score` FROM `players` WHERE `disconnected_at` IS NOT NULL AND `team` IS NOT NULL GROUP BY `team`", [], "Getting team scores", function(results){
			...
			for (team in team_scores) {
				if (team_scores.hasOwnProperty(team)){
					ordered_scores.push({
						team: team,
						score_seconds: parseFloat(team_scores[team].score.toFixed(3)),
			...
			Socket.io.sockets.emit("team_high_scores", ordered_scores.slice(0, 10));
		});
	},
	...
	send_top_spinners: function(){
		...
		function add_top_spinner(scores, initials, score_seconds, spm){
			scores.push({
				initials: initials,
				score_seconds: parseFloat(score_seconds.toFixed(3)),
				spm: spm,
			});
		}
		...
		for (var i = Game.players.length - 1; i >= 0; i--) {
			var player = Game.players[i];
			add_top_spinner(top_spinners, player.initials, player.score, player.spm);
		}

		// note that since we're only gonna send down the top 10, no need to get more than 5 here.
		DB.query("SELECT `score`, `initials` FROM `players` WHERE disconnected_at IS NOT NULL ORDER BY `score` DESC LIMIT 10", [], "getting high spinner scores", function(results){
			...
			Socket.io.sockets.emit("high_scores", top_spinners.slice(0, 10));
		});
	}
};
