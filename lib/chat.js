var Chat = {
	...
	enable: function(socket){
		...
		socket.on('message.team', function(data){
			...
			if (!data || !data.text || !socket.player.initials || !socket.player.team){ // can't send nothing nor send without initials.
				return;
			}

			// send to the same team as the 'from_team'
			Chat.send_team(socket.player, socket.player.team, data.text);
		});
